 ...
 *  - every window with flagFocusManager==true adds a sub domain to the top
 *    level domains' tree
 ...
 *    
 *  ATTENTION:
 *  Errors are bound to happen when the TWindow attributes `flagFocusManager'
...
bool new_key_eventhack = false;
TEventFilter * global_evt_filter = 0;
...
//---------------------------------------------------------------------------
void
TFocusManager::newWindow(TWindow* wnd)
{
//cout << "new window "<<wnd<<" '" << wnd->getTitle() << "'\n";
  assert(wnd!=NULL);
...
void
TFocusManager::destroyWindow(TWindow* wnd)
{
  assert(wnd!=NULL);
...
DBM(cout << "DomainToWindow " << (wnd ? wnd->getTitle() : "(NULL)") << endl;)
  ...
  if (wnd && !new_domain) {
    cerr << "warning: no key domain for window " << wnd << " '" << wnd->getTitle() << "'\n";
  }
 ...
 */
void
TFocusManager::setFocusWindow(TWindow* wnd)
{
  assert(wnd!=NULL);

//cout << "setFocusWindow " << wnd->getTitle() << endl;
  if (wnd->flagNoFocus && !wnd->flagFocusManager && !wnd->flagShell) {
/*
    cout << "window \"" << wnd->getTitle() << "\" doesn't need focus" << endl;
    cout << "  flagNoFocus     : " << (wnd->flagNoFocus?"true":"false") << endl;
    cout << "  flagFocusManager: " << (wnd->flagFocusManager?"true":"false") << endl;
...
ToggleDomain2(TInteractor *wnd, bool on)
{
  ...
  while(p) {
    if (!p->flagFocusManager && !p->flagShell)
      ToggleDomain2(p, on);
...
void
TFocusManager::handleEvent(TKeyEvent &keyevent)
{
  if (!current_domain) {
    cerr << "no key domain..." << endl;
    return;
  }
...
unsigned m = keyevent.modifier();
  ...
  if ( (key==TK_TAB || key==TK_LEFT_TAB) && 
        current_domain->focus_window &&
        current_domain->focus_window->flagTabKey )
    wnd = NULL;
  ...
  if (!current_domain->focus_window) {
    ...
    if (!wnd) { // // make exception for single toplevel window apps :(
      if (!current_domain->owner->flagNoFocus)
        wnd=current_domain->owner;
    }
    if (wnd) {
...
Walk(TWindow *top, TWindow *focus, bool bNext)
{
  ...
  do {
    ...
    if ( ptr->getFirstChild() &&      // go down when `ptr' has a child
         ...
         ptr->flagFocusTraversal &&   // and windows that don't want to be part of the focus traversal
         ptr->isRealized() &&         // and when the window is mapped
         ( ptr->flagNoFocus           // and only go down when window rejects focus
           || ptr->flagFocusManager   //   or when it's a focusmanager
    ...
    DBM(
      ...
      ptr->flagNoFocus ||          // continue when window doesn't want the focus
 ...
 */
void 
TFocusManager::insertEventFilter(TEventFilter *flt, TWindow *wnd, EEventFilterPos pos)
{
  flt->pos = pos;
  switch(pos) {
    case KF_GLOBAL:
 ...
 */
void
TFocusManager::removeEventFilter(TEventFilter *flt)
{
  switch(flt->pos) {
    case KF_GLOBAL:
